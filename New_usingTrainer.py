...
# )
# trainer.train()
import torch import torch.nn as nn import numpy as np import editdistance from transformers import ViTModel, AutoModelForCausalLM, AutoTokenizer, AutoConfig, TrainingArguments, Trainer, TrainerCallback from safetensors.torch import load_file, save_file from torchvision import transforms from torch.utils.data import Dataset from PIL import Image

--- ANLS Metric ---

def normalized_levenshtein(pred, gt): pred, gt = pred.strip().lower(), gt.strip().lower() if len(gt) == 0: return 1.0 if len(pred) == 0 else 0.0 dist = editdistance.eval(pred, gt) norm = dist / max(len(pred), len(gt)) return 1 - norm

def compute_anls(preds, gts, threshold=0.5): scores = [] for p, gt_list in zip(preds, gts): sims = [normalized_levenshtein(p, gt) for gt in gt_list] best_sim = max(sims) scores.append(best_sim if best_sim >= threshold else 0) return np.mean(scores)

--- Custom Model ---

class VisionLanguageModel(nn.Module): def init(self, vit_path="./vit", phi_path="./phi-2"): super().init()
...
class DummyDocVQADataset(Dataset): def init(self, tokenizer, num_samples=100): self.tokenizer = tokenizer self.data = [ { "image": torch.rand(3, 224, 224), "question": "What is the title?", "answers": ["Invoice", "Bill"] } for _ in range(num_samples) ]
...
def collate_fn(batch): pixel_values = torch.stack([item["pixel_values"] for item in batch]) questions = [item["question"] for item in batch] answers = [" ||| ".join(item["answers"]) for item in batch] return { "pixel_values": pixel_values, "questions": questions, "answers": answers }

--- Metric Function ---

def compute_metrics(eval_preds): predictions, labels = eval_preds decoded_preds = model.tokenizer.batch_decode(predictions, skip_special_tokens=True) decoded_labels = model.tokenizer.batch_decode(labels, skip_special_tokens=True) refs = [[gt.strip() for gt in label.split("|||")] for label in decoded_labels] score = compute_anls(decoded_preds, refs) return {"anls": score}

--- Save Best Checkpoint Callback ---

class SaveBestProjectionCallback(TrainerCallback): def init(self, monitor="anls", save_path="best_projector.safetensors"): self.monitor = monitor self.save_path = save_path self.best_score = -1
...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu") model = VisionLanguageModel().to(device) tokenizer = model.tokenizer

train_dataset = DummyDocVQADataset(tokenizer, num_samples=100) val_dataset = DummyDocVQADataset(tokenizer, num_samples=20)

args = TrainingArguments( output_dir="./vlm_checkpoints", per_device_train_batch_size=4, per_device_eval_batch_size=4, evaluation_strategy="epoch", save_strategy="epoch", logging_dir="./logs", num_train_epochs=3, fp16=True, save_total_limit=1, logging_steps=5, report_to="none" )

trainer = Trainer( model=model, args=args, train_dataset=train_dataset, eval_dataset=val_dataset, data_collator=collate_fn, tokenizer=tokenizer, compute_metrics=compute_metrics, callbacks=[SaveBestProjectionCallback()] )
